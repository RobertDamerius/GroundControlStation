...
RD::Network::Endpoint::Endpoint(){
    Reset();
}

RD::Network::Endpoint::Endpoint(uint8_t ipv4_A, uint8_t ipv4_B, uint8_t ipv4_C, uint8_t ipv4_D, uint16_t port){
    Reset();
...
void RD::Network::Endpoint::Reset(void){
...
void RD::Network::Endpoint::SetAddress(sockaddr_in address){
...
void RD::Network::Endpoint::SetAddress(in_addr address){
    ENDPOINT_IP(addr) = address.s_addr;
    UpdateID();
}

void RD::Network::Endpoint::SetPort(uint16_t port){
    ENDPOINT_PORT(addr) = htons(port);
    UpdateID();
}

void RD::Network::Endpoint::SetIPv4(uint8_t ipv4_A, uint8_t ipv4_B, uint8_t ipv4_C, uint8_t ipv4_D){
    ENDPOINT_IP(addr) = htonl((0xFF000000 & (ipv4_A << 24)) | (0x00FF0000 & (ipv4_B << 16)) | (0x0000FF00 & (ipv4_C << 8)) | (0x000000FF & ipv4_D));
    UpdateID();
}

uint16_t RD::Network::Endpoint::GetPort(void){
    return (uint16_t)ntohs(ENDPOINT_PORT(addr));
}

void RD::Network::Endpoint::GetIPv4(uint8_t *ipv4){
    if(ipv4){
...
std::string RD::Network::Endpoint::ToString(void){
    char buf[24];
    uint8_t ipv4[4];
    uint32_t u32 = ntohl(ENDPOINT_IP(addr));
    ipv4[0] = (uint8_t)(0x000000FF & (u32 >> 24));
...
bool RD::Network::Endpoint::DecodeFromString(std::string strEndpoint){
    ...
    if(strPort.size()){
        ...
        if((P < 0) || (P > 65535))
            return false;
        RD::Network::Endpoint::SetPort((uint16_t)P);
    }
    RD::Network::Endpoint::SetIPv4((uint8_t)A, (uint8_t)B, (uint8_t)C, (uint8_t)D);
    UpdateID();
    return true;
}

bool RD::Network::Endpoint::CompareIPv4(const uint8_t ipv4_A, const uint8_t ipv4_B, const uint8_t ipv4_C, const uint8_t ipv4_D){
    uint32_t u32 = ntohl(ENDPOINT_IP(addr));
...
bool RD::Network::Endpoint::CompareIPv4(const RD::Network::Endpoint& endpoint){
    return (ENDPOINT_IP(this->addr) == ENDPOINT_IP(endpoint.addr));
}

bool RD::Network::Endpoint::ComparePort(const uint16_t port){
    return (port == (uint16_t)(ntohs(ENDPOINT_PORT(this->addr))));
}

bool RD::Network::Endpoint::ComparePort(const RD::Network::Endpoint& endpoint){
    return (ENDPOINT_PORT(this->addr) == ENDPOINT_PORT(endpoint.addr));
}

RD::Network::Endpoint& RD::Network::Endpoint::operator=(const RD::Network::Endpoint& rhs){
    if(this != &rhs){
...
bool RD::Network::Endpoint::operator==(const RD::Network::Endpoint& rhs)const{
...
void RD::Network::Endpoint::UpdateID(void){
    uint32_t u32 = ntohl(ENDPOINT_IP(addr));
